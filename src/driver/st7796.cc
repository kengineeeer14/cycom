...
ST7796::ST7796(hal::ISpi *spi, hal::IGpio *dc, hal::IGpio *rst, hal::IGpio *bl) : spi_(spi), dc_(dc), rst_(rst), bl_(bl) {
...
void ST7796::BlitRGB565(const uint8_t *buf, size_t len) {
    ...
    if (len != expected)
        throw std::runtime_error("BlitRGB565: size mismatch");
    SetAddressWindow(0, 0, kWidth - 1, kHeight - 1);
    DataMode(true);
    SendChunked(buf, len);
}
...
void ST7796::Init() {
    ...
    for (uint8_t v : std::array<uint8_t, 8>{0x40, 0x8A, 0x00, 0x00, 0x29, 0x19, 0xA5, 0x33})
        Dat(v);
    Cmd(0xE0);
    for (uint8_t v : std::array<uint8_t, 14>{0xD0, 0x08, 0x0F, 0x06, 0x06, 0x33, 0x30, 0x33, 0x47, 0x17, 0x13, 0x13, 0x2B, 0x31})
        Dat(v);
    Cmd(0xE1);
    for (uint8_t v : std::array<uint8_t, 14>{0xD0, 0x0A, 0x11, 0x0B, 0x09, 0x07, 0x2F, 0x33, 0x47, 0x38, 0x15, 0x16, 0x2C, 0x32})
        Dat(v);
