...
GT911::GT911(hal::II2c *i2c, hal::IGpio *rst, hal::IGpio *int_pin, uint8_t i2c_addr) : i2c_(i2c), rst_(rst), int_pin_(int_pin), i2c_addr_(i2c_addr) {
...
void GT911::ReadTouchData(TouchPoint &point) {
    ...
    if (x < 0)
        x = 0;
    else if (x >= kCoordinateXMax)
        x = kCoordinateXMax - 1;
    if (y < 0)
        y = 0;
    else if (y >= kCoordinateYMax)
        y = kCoordinateYMax - 1;
